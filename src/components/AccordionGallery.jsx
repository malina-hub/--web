...
const AccordionGallery = ({
  items = [], defaultIndex = 2, accentColor = '#ffffff',
  overlayColor = '#060010', textColor = '#ffffff', height = 460,
  gap = 10, radius = 16, expandRatio = 0.52, orientation = 'horizontal',
  duration = 0.6, ease = 'power3.out', parallax = 0.5, tilt = 8,
  stagger = 0.06, trigger = 'hover', showLabels = true,
  grayscale = true, className = ''
}) => {
  ...
  const applyLayout = useCallback(animate => {
    ...
    panels.forEach((panel, i) => {
      ...
      if (media) {
        ...
        tl.to(media, { xPercent:-50, yPercent:-50, x:vertical?0:isActive?0:shift,
          y:vertical?(isActive?0:shift):0, '--ag-gray':grayscale?(isActive?0:1):0,
          '--ag-dim':isActive?0:0.35, duration:dur, ease }, 0);
      }
      if (showLabels && bar && text) {
        if (isActive) tl.to([bar,text], { opacity:1, x:0, duration:dur, ease, stagger:prefersReduced?0:stagger }, 0);
        else tl.to([bar,text], { opacity:0, x:-14, duration:dur*.6, ease }, 0);
      }
    });
    tlRef.current = tl;
  ...
  const handleClick = (i,e) => {
    if (items[i]?.link === '#' || i !== active) e.preventDefault();
  ...
  return <div ref={rootRef}
    ...
    style={{'--ag-accent':accentColor,'--ag-overlay':overlayColor,'--ag-text':textColor,
      '--ag-gap':`${gap}px`,'--ag-radius':`${radius}px`,height:vertical?`${Math.round(height*1.6)}px`:`${height}px`}}
    role="list" aria-label="影视工业沉浸式体验图片">
    {items.map((item,i) => {
      const isActive=i===active; const Tag=item.link?'a':'div';
      return <Tag key={i} ref={el=>panelRefs.current[i]=el}
        className={`ag-panel${isActive?' ag-panel--active':''}`} style={{borderRadius:`${radius}px`}}
        href={item.link||undefined} onClick={e=>handleClick(i,e)}
