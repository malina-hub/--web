...
void main() {
  ...
  if (uColorCount > 0) {
    ...
    for (int i = 0; i < MAX_COLORS; ++i) {
      ...
      float weight = 1.0 - exp(-uBandWidth / exp(uBandWidth * m));
...
const toVec3 = value => {
  ...
  return new THREE.Vector3(...rgb.map(channel => Number.parseInt(channel, 16) / 255));
};
...
export default function ColorBends({
  ...
  const valuesRef = useRef({ rotation, autoRotate });
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const radians = ((valuesRef.current.rotation % 360) + valuesRef.current.autoRotate * elapsed) * Math.PI / 180;
  ...
  useEffect(() => {
    ...
    if (!material) return;
    valuesRef.current = { rotation, autoRotate };
    ...
    const converted = colors.filter(Boolean).slice(0, MAX_COLORS).map(toVec3);
    material.uniforms.uColors.value.forEach((color, index) => color.copy(converted[index] || new THREE.Vector3()));
    material.uniforms.uColorCount.value = converted.length;
